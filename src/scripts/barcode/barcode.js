...
class barcodeScanner {
  constructor(node) {
    ...
    this.render();
    this.audio = new Audio('/assets/sounds/scan.wav');
  }
      ...
      barcodeDetector
        ...
        .then((barcodes) => {
          ...
          let boundingBoxes = barcodes.map((barcode, i) => {
            if (barcode.rawValue) {
              barcodeCheck = true;
              document.querySelector('.support__boolean').textContent =
                barcode.rawValue;
          ...
          if (barcodeCheck) {
            ...
            context.rect(
              gecombineerdObject.x,
              gecombineerdObject.y,
              gecombineerdObject.width,
              gecombineerdObject.height
            );
            context.fill();
            // Pause video
            this.video.pause();
            ...
            this.controlRedirect().addEventListener('click', () => {
              ...
              this.controlRedirect().href = `/producten/${
                this.controlRedirect().value
              }`;
